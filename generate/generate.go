...
func (as *allServices) GeneralCode() ([]byte, error) {
	...
	pn("")
	pn("    // if GO_CLOUDSTACK_ALWAYS_USE_GET == 1 always use GET method")
	pn("    f := os.Getenv(\"GO_CLOUDSTACK_ALWAYS_USE_GET\")")
	pn("")
	pn("    if (api == \"deployVirtualMachine\" || api == \"updateVirtualMachine\") && f != \"1\" {")
