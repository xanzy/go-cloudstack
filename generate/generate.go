...
func (s *service) generateHelperFuncs(a *API) {
	...
	if strings.HasPrefix(a.Name, "list") {
		...
		if found := hasIDParamField(a.Params); found {
			...
			pn("		p.p[\"projectid\"] = \"-1\"")
			pn("		l2, err := s.ListVirtualMachines(p)")
			pn("		if err != nil {")
			pn("			if strings.Contains(err.Error(), fmt.Sprintf(")
			pn("				\"Invalid parameter id value=%%s due to incorrect long value format, \"+")
			pn("				\"or entity does not exist\", id)) {")
			pn("				return nil, 0, fmt.Errorf(\"No match found for %%s in any project: %%+v\", id, l)")
			pn("			}")
			pn("			return nil, -1, err")
			pn("		}")
			pn("		if l2.Count == 0 {")
			pn("			return nil, l.Count, fmt.Errorf(\"No match found for %%s in any project: %%+v\", id, l)")
			pn("		}")
			pn("		l.Count += l2.Count")
			pn("		l.VirtualMachines = append(l.VirtualMachines, l2.VirtualMachines...)")
			pn("	}")
			pn("")
			pn("	if l.Count == 0 {")
